...
export default function FillInBlanks({ exercise, onResult }: Props) {
  ...
  async function handleRun() {
    ...
    try {
      ...
      setOutput({ stdout: result.stdout, stderr: result.stderr || (result.timed_out ? 'Timed out!' : '') })
    } finally {
      setRunning(false)
    }
  }
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      if (passed) {
        addXPEvent(xp_gained, 'Exercise passed!')
    ...
    <div className="space-y-4">
      <div className="card p-5">
        <div className="flex items-center gap-2 mb-3">
          <span className="badge-primary">Fill in the Blanks</span>
          <span className="text-xs text-yellow-500 font-medium">+{exercise.xp_reward} XP</span>
        </div>
        <p className="text-gray-700 dark:text-gray-300 mb-4 whitespace-pre-wrap">{exercise.instructions}</p>
        ...
        {hintIndex >= 0 && (
          <div className="mt-3 bg-amber-50 dark:bg-amber-900/20 rounded-xl p-3 text-sm text-amber-700 dark:text-amber-300">
            💡 Hint {hintIndex + 1}: {exercise.hints[hintIndex]}
          </div>
        )}
      </div>
    </div>
